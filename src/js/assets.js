...
µBlock.assets = (function() {
    ...
    var readRepoCopyAsset = function(path, callback) {
        ...
        var onHomeFileLoaded = function(details) {
            cachedAssetsManager.save(path, this.responseText, callback);
        }
        var onHomeFileError = function() {
            reportBack('', 'Error');
        }
        var onLocalFileLoaded = function(details) {
            reportBack(this.responseText);
        };
        var onLocalFileError = function(details) {
            getTextFileFromURL(homeURL, onHomeFileLoaded, onHomeFileError);
        };
        var fetchContent = function() {
            let entry = µb.remoteBlacklists[path];
            if(entry) {
                if(entry.assetURL) {
                    let assetURL = 'assets/thirdparties/' + entry.assetURL;
                    getTextFileFromURL(vAPI.getURL(assetURL), onLocalFileLoaded, onLocalFileError);
                } else {
                    getTextFileFromURL(homeURL, onHomeFileLoaded, onHomeFileError);
                }
            } else {
        ...
        var onCacheMetaReady = function(entries) {
            ...
            // In cache
            if ( inCache ) {
                //console.log('µBlock> Cache data is available: "%s"', path);
                cachedAssetsManager.load(path, onCachedContentLoaded, onCachedContentError);
                return;
            }
            
            //console.log('µBlock> Cache data is not available: "%s"', path);
            fetchContent();
        };
        cachedAssetsManager.entries(onCacheMetaReady);
    };

    var readRemoteAssets = function(path, callback) {
        ...
        if ( stringIsNotEmpty(homeURL) === false ) {
            reportBack('', 'Error');
        }
    ...
    var readRepoOnlyAsset = function(path, callback) {
        ...
        var onInstallFileLoaded = function() {
            //console.log('readRepoOnlyAsset >> µBlock> readRepoOnlyAsset("%s") / onInstallFileLoaded()', path);
            reportBack(this.responseText);
        };
    
        var onInstallFileError = function() {
            //console.error('readRepoOnlyAsset >> µBlock> readRepoOnlyAsset("%s") / onInstallFileError()', path);
            reportBack('', 'Error');
        };
    
        var onCachedContentLoaded = function(details) {
            //console.log('readRepoOnlyAsset >> µBlock> readRepoOnlyAsset("%s") / onCachedContentLoaded()', path);
            reportBack(details.content);
        };
    
        var onCachedContentError = function() {
            //console.log('readRepoOnlyAsset >> µBlock> readRepoOnlyAsset("%s") / onCachedContentError()', path);
    ...
    exports.getRemote = readRemoteAssets;
    ...
    exports.metadata = function(callback) {
        ...
        var onRepoMetaReady = function() {
            var entries = JSON.parse(JSON.stringify(µb.remoteBlacklists));
    ...
    var updateOne = function() {
        ...
        for ( var path in toUpdate ) {
           ...
           if ( metadata.hasOwnProperty(path) === false ) {
            ...
            µb.assets.getRemote(path, onOneUpdated);
            updatingCount = 1;
